	...
	try:
		...
		for row in reader:
			...
			if service_id == '1':
				...
				channel.basic_publish(exchange='', routing_key=INSULT_QUEUE, body=message)
			elif service_id == '2':
				# Add text to texts rabbitmq
				order = {
					'text': row.get('Text')
				}
				print("Processing order:", order)
				...
				print(f"Skipping order with unknown ServiceID: {service_id}")
				continue
